    ...
    namespace SiteChecker
    {
        class CheckStarter
        {
            ...
            static void initWebsites()
            {
                ...
                {
                    Website site = new Website(configWebsites[idx], configCookies[idx], configSounds[idx]);
                    websites.Add(site);
                }
            }
            ...
            static HTMLElement parseHTMLData(HTMLData htmlData)
            {
                ...
                string text = TagsToSkip.Contains(currentNode.TagType) ? "" : parseText(htmlData.Data);
